...
import ListRow from '@polkadot/ui-app/ListRow';
...
class ListAddress extends React.PureComponent<Props, State> {
  ...
  static getLastValue (type: KeyringOption$Type = DEFAULT_TYPE) {
    const options = ListAddress.readOptions();
  ...
  render () {
    const { className, defaultValue, hideAddress = false, isDisabled = false, isError, label, optionsAll, type = DEFAULT_TYPE, style, withLabel, showEditPanel, showForgetPanel } = this.props;
    ...
    const lastValue = ListAddress.getLastValue(type);
    ...
    const optionsList =  isDisabled && actualValue ? [makeOption(actualValue)] : optionsAll[type];
    ...
    return (
      <div className="customAddressList">
        {optionsList.map((address, index) => (
        index > 0 
        ? 
        <div className="customAddressListRow">
          <ListRow
            key={address.value || ''}
            value={address.value || ''}
            name={address.name || ''}
            ...
            withNonce={false}
          />
          <Button.Group className="customAddressButtons">
          <Button
            onClick={() => this.props.showForgetPanel(address.value)}
            isNegative
            text='Forget'
          />
          <Button.Or />
          <Button
            onClick={() => this.props.showEditPanel(address.value)}
            isPrimary
            text='Edit'
          />
        </Button.Group>
  ...
  private onChange = (address: string) => {
    ...
    ListAddress.setLastValue(type, address);

    onChange(transform(address));
  }

  private onEdit = (address: any): void => {
    const { onChange, type } = this.props;
  ...
  private onForget = (address: any): void => {
    console.log("Forget?");
  }
...
export default withObservableBase(optionsSubject, { propName: 'optionsAll' })(ListAddress);
